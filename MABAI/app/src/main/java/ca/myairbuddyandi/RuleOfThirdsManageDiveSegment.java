 ...
 * Hold all of the logic for the RuleOfThirdsManageDiveSegment class
 */


public class RuleOfThirdsManageDiveSegment {

    // Static
    private static final String LOG_TAG = "RuleOfThirdsManageDiveSegment";
    ...
    // Public constructor
    public RuleOfThirdsManageDiveSegment(Context context) {
        mContext = context;
    }
    ...
    Double generateDiveSegment(
        ...
        airDa.insertDiveSegmentRuleOfThird(
                 diverNo
                ,diveNo
        ...
        double twoThirdPressure = beginningPressure * 0.666666;
        Double sumAscentPressure = MyConstants.ZERO_D;
        Double sumDescentPressure = MyConstants.ZERO_D;

        // Forced BottomTime is used when 2 divers have different bottom times
        // The diver with the highest bottom time will be forced to the same bottom time of the diver with the lowest bottom time
        ...
        for (int i=0;i<diveSegmentList.size();i++)
        {
            ...
            if (i < iBtBeforeTa) {
                // All of the following segments are of type Descent, excluding Bottom Times
                sumDescentPressure += diveSegment.getAirConsumptionPressure();
            }

            if (i > iBTAfterTa) {
                // All of the following segments are of type Ascent, excluding Bottom Times
                sumAscentPressure += diveSegment.getAirConsumptionPressure();
            }
        ...
        Double taPressure = MyFunctionsSegments.getTAPressure(diveSegmentList,iTa);
        ...
        switch(optimize) {
            ...
            case "Plus": // Maximize the Bottom Times to fit within the dive parameters
                ...
                // Setting the Bottom Time (BT) BEFORE the Turnaround (TA)
                //                              ======
                diveSegment = diveSegmentList.get(iBtBeforeTa);

                diveSegment.setAirConsumptionPressure((twoThirdPressure - sumDescentPressure - sumAscentPressure - taPressure) / 2);
                ...
                // Setting the Bottom Time (BT) AFTER the Turnaround (TA)
                //                              =====
                diveSegment = diveSegmentList.get(iBTAfterTa);
        ...
        // Update Dive Segments back in the DB
        for (int i=0;i<diveSegmentList.size();i++)
        {
            diveSegment = diveSegmentList.get(i);
