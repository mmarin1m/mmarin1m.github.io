 ...
 * Comparator to sort on Acronym Description
 *
 * To sort Computer by Description
 */

public class ComputerPickComparatorDescription implements Comparator<ComputerPick> {

    // Static
    private static final String LOG_TAG = "ComputerPickComparatorDescription";
    ...
    // Public constructor
    public ComputerPickComparatorDescription(boolean desc) {
        if (desc) mod =-1;
    }

    public int compare(ComputerPick c1, ComputerPick c2) {
        Matcher matcher1 = PATTERN.matcher(c1.getDescription());
        Matcher matcher2 = PATTERN.matcher(c2.getDescription());
