 ...
 * Comparator to sort on GrouppPick Type
 */

public class GrouppPickComparatorType implements Comparator<GrouppPick> {

    // Static
    private static final String LOG_TAG = "GrouppPickComparatorType";
    ...
    // Public constructor
    public GrouppPickComparatorType(boolean desc) {
        if (desc) mod =-1;
    }

    public int compare(GrouppPick c1, GrouppPick c2) {
        Matcher matcher1 = PATTERN.matcher(c1.getGroupType());
        Matcher matcher2 = PATTERN.matcher(c2.getGroupType());
