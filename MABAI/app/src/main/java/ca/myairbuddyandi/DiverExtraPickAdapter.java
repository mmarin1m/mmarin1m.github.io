 ...
 * Holds all of the logic for the DiverExtraPickAdapter class
 */

public class DiverExtraPickAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements Filterable {

    // Static
    private static final String LOG_TAG = "DiverExtraPickAdapter";
    ...
    private DiverExtraPickFilter mFilter;

    // End of variables

    // nPublic constructor
    public DiverExtraPickAdapter(Context context, ArrayList<Diver> diverPickList) {
        mContext = context;
    ...
    private class VHItem extends DiverExtraPickAdapter.ViewHolder {
        ...
        public VHItem(View itemView) {
            ...
            binding.detailIcon.setOnClickListener(view -> {
                ...
                ((DiverExtraPickActivity) mContext).setDiver(mDiver);
                ((DiverExtraPickActivity) mContext).editLauncher.launch(intent);
            });
            ...
            itemView.setOnClickListener(view -> {
                if (mInMultiEditMode) {
                    ...
                    ((DiverExtraPickActivity) mContext).countDivers(checkBox.isChecked());
                } else {
            ...
            itemView.setOnLongClickListener(view -> {
                ...
                if (position >= MyConstants.ZERO_I) {
                    mExpandedPosition = -1;
                    ((DiverExtraPickActivity) mContext).setVisibility(position, mInMultiEditMode);
                }
                return true;
            });
        }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == TYPE_HEADER) {
            ...
            return new DiverExtraPickAdapter.VHHeader(pickDiverAdapter);
        } else if (viewType == TYPE_ITEM) {
            View pickDiverAdapter = LayoutInflater.from(parent.getContext()).inflate(R.layout.diver_pick, parent, false);
            return new DiverExtraPickAdapter.VHItem(pickDiverAdapter);
        }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof DiverExtraPickAdapter.VHItem) {
            Diver dataObject = mDiverPickList.get(position - HEADER_OFFSET);

            DiverExtraPickAdapter.ViewHolder itemView = (DiverExtraPickAdapter.ViewHolder)  holder;
            ...
            if (position == mExpandedPosition) {
                ((DiverExtraPickAdapter.VHItem) holder).expandedArea.setVisibility(View.VISIBLE);
                if (position == mDiverPickList.size()) {
                    // Last row
                    // Scroll to the bottom to show the icon bar
                    ((DiverExtraPickActivity) mContext).doSmoothScroll(mDiverPickList.size());
                }
            } else {
                ((DiverExtraPickAdapter.VHItem) holder).expandedArea.setVisibility(View.GONE);
            }
            ...
            ((DiverExtraPickAdapter.VHItem) holder).letterCircle.setImageDrawable(circle);

            ((DiverExtraPickAdapter.VHItem) holder).checkBox.setOnClickListener(v -> {
    ...
    public Filter getFilter() {
        if(mFilter==null)
        {
            mFilter=new DiverExtraPickFilter(mDiverPickList,this);
        }
        return mFilter;
    }
    ...
    public void addDiver(Diver diver) {
        ...
        ((DiverExtraPickActivity) mContext).doSmoothScroll(getItemCount());
