 ...
 * Hold all of the logic for the TurnaroundManageDiveSegment class
 */

public class TurnaroundManageDiveSegment {

    // Static
    private static final String LOG_TAG = "TurnaroundManageDiveSegment";
    ...
    // Public constructor
    public TurnaroundManageDiveSegment(Context context) {
        mContext = context;
    }
    ...
    Double generateDiveSegment(
        ...
        airDa.insertDiveSegmentTurnaround(
                 diverNo
                ,diveNo
        ...
        int iTa = MyFunctionsSegments.findTurnaroundSegment(mDiveSegmentList);
        ...
        if (minimumTimeBetweenTwoDivers .equals(MyConstants.ZERO_D)) {
            // First time in OR both divers have the same BT
            // A planned BT of 10 minutes, becomes 2 BT of 5 minutes; to and back
            diveSegment = mDiveSegmentList.get(iBtBeforeTa);
            diveSegment.setMinute(diveSegment.getMinute() / 2);
            mDiveSegmentList.set(iBtBeforeTa, diveSegment);

            diveSegment = mDiveSegmentList.get(iBTAfterTa);
            diveSegment.setMinute(diveSegment.getMinute() / 2);
            mDiveSegmentList.set(iBTAfterTa, diveSegment);
        } else {
            // Forcing smallest BT between the two divers
            // An already calculated BT of 8 minutes, becomes 2 BT of 4 minutes; to and back
            diveSegment = mDiveSegmentList.get(iBtBeforeTa);
            diveSegment.setMinute(minimumTimeBetweenTwoDivers / 2);
            mDiveSegmentList.set(iBtBeforeTa, diveSegment);
        ...
        // deltaPressure represents the extra or missing psi for the last Bottom Time
        // A deltaPressure of 500 psi represents an extra 250 psi; to and back
        // A deltaPressure of -500 psi represents a missing 250 psi; to and back
        Double deltaPressure = (beginningPressure - myMinPressure - sumPressure) / 2;
        Double totalOptimizedBottomTime = MyConstants.ZERO_D;

        // Start optimizing the Bottom Times as per user's request
        switch(optimize) {
            case "Minus": // Minimize the Bottom Times to fit within the dive parameters

                // Minimize the Bottom Times only if there is not enough air capacity to complete the dive
                // After minimizing the Bottom Times, it is still possible that a diver will not have enough air capacity
            ...
            case "Plus": // Maximize the Bottom Times to fit within the dive parameters

                // Optimize the Bottom Times only if there is room for improvements
                if (deltaPressure <= MyConstants.ZERO_D) {
                    break;
                }
